...
function getCurrentWord() {
  return state.grid[state.cRow].reduce((prev, curr) => prev + curr);
}
...
function addLetter(letter) {
  if (state.cCol === 4) return;
  state.grid[state.cRow][state.cCol] = letter;
  state.cCol++;
}

function removeLetter() {
  if (state.cCol === 0) return;
  state.grid[state.cRow][state.cCol - 1] = '';
  state.cCol--;
}
